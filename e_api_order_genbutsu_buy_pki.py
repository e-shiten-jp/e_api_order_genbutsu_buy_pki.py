...
if __name__ == "__main__":
    ...
    # コマンド用パラメーター -------------------    
    # 現物 買い 注文パラメーターセット
    my_sIssueCode = S_ISSUE_CODE  # 10.銘柄コード。実際の銘柄コードを入れてください。
    my_sSizyouC = S_SIZYOU_C      # 11.市場。  00:東証   現在(2021/07/01)、東証のみ可能。
    my_sCondition = S_CONDITION   # 13.執行条件。  0:指定なし、2:寄付、4:引け、6:不成。指し値は、0:指定なし。
    my_sOrderPrice = S_ORDER_PRICE  # 14.注文値段。  *:指定なし、0:成行、上記以外は、注文値段。小数点については、関連資料:「立花証券・e支店・API、REQUEST I/F、マスタデータ利用方法」の「2-12. 呼値」参照。
    my_sOrderSuryou = S_ORDER_SURYOU # 15.注文数量。
